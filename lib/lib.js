...
function pgzUi(key, params, fallback) {
...
var PythonIDE = {
    ...
    debugHandler: function(result) {
        ...
        if(PythonIDE.breakpoints[filename] && PythonIDE.breakpoints[filename][lineNumber] && PythonIDE.continueRunning) {
            PythonIDE.showHint(pgzUi('ide.breakpointPaused', { line: lineNumber }, 'Приостановлено из-за точки останова на строке ' + lineNumber));
        ...
        if(true || PythonIDE.runMode == "step" || breakpoint) {
            var html = '<i id="btnToggleGlobals" class="fa fa-arrows-h"></i> <div id="globals"><h3>' + pgzUi('ide.globalsTitle', null, 'Глобальные переменные: ') + '</h3><table><tr><th>' + pgzUi('ide.colName', null, 'Имя') + '</th><th>' + pgzUi('ide.colType', null, 'Тип данных') + '</th><th>' + pgzUi('ide.colValue', null, 'Значение') + '</th></tr></div>';
            ...
            for(var key in context.$gbl) {
                ...
                if(val && val.length && val.length > 20) {
                    ...
                    val = '<span class="debug_expand_zone" id="debug_expand_' + eH.id + '">' + val.substring(0, 17) + '<img src="./media/tools.png" class="debug_expand" title="' + pgzUi('ide.expandValue', null, 'Нажмите, чтобы увидеть значение целиком') + '"></span>';
                }
            ...
            while(context && context.$tmps && context.$lineno) {
                html += '<h4><span>' + pgzUi('ide.localsAtLine', null, 'Локальные переменные в строке') + '</span> ' + context.$lineno + '</h4><table><tr><th>' + pgzUi('ide.colName', null, 'Имя') + '</th><th>' + pgzUi('ide.colType', null, 'Тип данных') + '</th><th>' + pgzUi('ide.colValue', null, 'Значение') + '</th></tr>';
    ...
    editFile: function(filename, lineno) {
        ...
        if(filename != PythonIDE.currentFile) {
            ...
            if (typeof EditorIDE !== 'undefined') EditorIDE.updateTabName(filename);
    ...
    refreshEditorView: function() {
        if (!PythonIDE.editor) return;
        var editor = PythonIDE.editor;
    ...
    updateFileTabs: function() {
        ...
        for(var file in PythonIDE.files){
            ...
            if((file == PythonIDE.currentFile) || (file == PythonIDE.projectName + ".py") && (PythonIDE.currentFile == "my_pgz.py")) {
                ...
                if(file != 'my_pgz.py') {
                    html += '<img class="btn_file_settings" alt="' + pgzUi('ide.fileSettings', null, 'Настройки файла') + '" title="' + pgzUi('ide.fileSettings', null, 'Настройки файла') + '" src="./media/settings.png">';
                }
            } else {
                html += '">';
            }
        ...
        html += '<span class="file_tab"><img class="btn_file_settings" '
            + 'alt="' + pgzUi('ide.newFile', null, 'Создать новый файл') + '" title="' + pgzUi('ide.newFile', null, 'Создать новый файл') + '"'
    ...
    showHint: function(msg) {
        ...
        if (typeof PZTooltip !== 'undefined') PZTooltip.hide();
    },
            ...
            if(code == undefined) {
                ...
                while(r.$isSuspension) {
                    ...
                    if(now - startTime > 5000) {
                        PythonIDE.showHint(pgzUi('ide.stoppedTimeout', null, 'Остановлено через 5 секунд, чтобы избежать сбоя в работе браузера'));
                        break;
                    }
                }
    ...
    vault: localStorage.vault ? JSON.parse(localStorage.vault) : [],
        ...
        if (recoverCountText) {
            var countTpl = (typeof PGZI18n !== 'undefined' && PGZI18n.uiText)
                ? PGZI18n.uiText('recoverModal.count', { n: '{COUNT}' }, 'Количество резервных копий кода, которые сейчас хранятся в хранилище: {COUNT}')
                : 'Количество резервных копий кода, которые сейчас хранятся в хранилище: {COUNT}';
            recoverCountText.innerHTML = countTpl.replace('{COUNT}', '<span id="vault_count">' + PythonIDE.vault.length + '</span>');
        }
        ...
        if (sliderRecover && sliderRecover.tagName === 'INPUT' && sliderRecover.type === 'range') {
            ...
            sliderRecover.addEventListener('input', function() {
                ...
                var savedAtLabel = (typeof PGZI18n !== 'undefined' && PGZI18n.uiText)
                    ? PGZI18n.uiText('recoverModal.savedAt', null, 'Время сохранения:')
                    : 'Время сохранения:';

                var timeHtml = '<p class="recover_date">' + dateStr + '</p>';
                timeHtml += '<p class="recover_label">' + savedAtLabel + '</p>';
        ...
        if (btnRecoverClear) {
            ...
            btnRecoverClear.addEventListener('click', function() {
                if (confirm(pgzUi('recoverModal.deleteAllConfirm', null, 'Вы уверены, что хотите удалить все резервные копии? Это действие нельзя отменить.'))) {
                    PythonIDE.vault = [];
                    ...
                    if (recoverTimeEl) {
                        var noBackups = (typeof PGZI18n !== 'undefined' && PGZI18n.uiText)
                            ? PGZI18n.uiText('recoverModal.noBackups', null, 'Резервных копий нет.')
                            : 'Резервных копий нет.';
                        recoverTimeEl.innerHTML = noBackups;
                    }
                }
            });
        }
        ...
        if (PythonIDE.vault.length > 0) {
            ...
            const codePreview = document.querySelector('#code_preview code');
            if (codePreview) {
                codePreview.textContent = pgzUi('recoverModal.noBackupsComment', null, '# Резервных копий нет\n# Запустите код, чтобы создать резервную копию');
            }
        ...
        if (typeof PGZCodeHistory !== 'undefined') {
            PGZCodeHistory.pushFromIDE('run');
            return;
        }
        if(submit === undefined) {
            submit = true;
...
runCode: function(runMode) {
    ...
    var filesystem = window.jsfs;
    // Сброс кеша картинок перед каждым запуском — очищаем объект на месте,
    // иначе pgzrun держит ссылку на старый PGZ_IMAGE_CACHE и не видит новые спрайты.
    if (typeof clearRuntimeImageCache === 'function') {
        clearRuntimeImageCache();
    } else if (window.PGZ_IMAGE_CACHE) {
        Object.keys(window.PGZ_IMAGE_CACHE).forEach(function (key) { delete window.PGZ_IMAGE_CACHE[key]; });
        window.PGZ_PRELOAD_COMPLETE = false;
    } else {
        window.PGZ_IMAGE_CACHE = {};
        window.PGZ_PRELOAD_COMPLETE = false;
    }
    if (window.PGZ_IMAGE_PROMISES) {
        Object.keys(window.PGZ_IMAGE_PROMISES).forEach(function (key) { delete window.PGZ_IMAGE_PROMISES[key]; });
    } else {
        window.PGZ_IMAGE_PROMISES = {};
    }
    if (filesystem && typeof filesystem.invalidateMetadataCache === 'function') {
        filesystem.invalidateMetadataCache();
    }

    const IMAGE_EXTENSIONS = ['.png', '.jpg', '.jpeg', '.gif', '.webp'];

    function imageCacheKey(filename) {
        return filename.replace(/\.(png|jpe?g|gif|webp)$/i, '');
    }

    function loadImageGlobal(name, filePath) {
        ...
        function readImage(path) {
            return filesystem.read(path).then(data => new Promise((resolve, reject) => {
                const img = new Image();
                img.onload = () => {
        ...
        const candidates = filePath
            ? [filePath]
            : IMAGE_EXTENSIONS.map(ext => '/images/' + name + ext);

        const p = (async () => {
            for (let i = 0; i < candidates.length; i++) {
                try {
                    return await readImage(candidates[i]);
                } catch (err) {
    ...
    function preloadImages() {
        ...
        if (!filesystem || typeof filesystem.ls !== 'function') {
            window.PGZ_PRELOAD_COMPLETE = true;
            return Promise.resolve();
        }
        return filesystem.ls("/images").then(files => {
            if (!files) return;
            const tasks = [];
            var skipped = [];
            files.forEach(file => {
                ...
                if (!IMAGE_EXTENSIONS.some(ext => lower.endsWith(ext))) {
                    skipped.push(file);
                    console.warn('Preload skip unsupported image format:', file);
                    return;
                }
                const name = imageCacheKey(file);
                console.log("Preload: ", name);
                tasks.push(loadImageGlobal(name, '/images/' + file));
            });
            return Promise.all(tasks).then(() => {
                window.PGZ_PRELOAD_COMPLETE = true;
                console.log("Preload complete!");
                if (skipped.length && typeof PythonIDE !== 'undefined' && PythonIDE.showHint) {
                    var supported = (typeof PGZImageAssets !== 'undefined' && PGZImageAssets.SUPPORTED_FORMATS_LABEL)
                        ? PGZImageAssets.SUPPORTED_FORMATS_LABEL
                        : 'PNG, JPG, GIF, WEBP';
                    PythonIDE.showHint(pgzUi('assets.unsupportedImagesInProject', {
                        names: skipped.join(', '),
                        supported: supported
                    }, 'В проекте есть картинки неподходящего формата: ' + skipped.join(', ') + '. Нужны ' + supported + '.'));
                }
            });
        });
    }
    ...
    const start = () => {
        ...
        if(!PythonIDE.files['my_pgz.py']) {
            PythonIDE.files['my_pgz.py'] = pgzUi('ide.needMainFile', null, '# Нужен файл с названием my_pgz.py');
            PythonIDE.updateFileTabs();
        }
        if(!PythonIDE.files[PythonIDE.currentFile]) {
        ...
        PythonIDE.showHint(pgzUi('ide.running', null, 'Программа выполняется...'));
        ...
        Sk.misceval.callsimAsync(handlers, function() {
            ...
            PythonIDE.showHint(pgzUi('ide.finished', null, 'Программа завершила выполнение.'));
            ...
            if (gameModal) {
                if (window.GameModal) window.GameModal.close();
                else gameModal.style.display = "none";
            }
    ...
    // PRELOAD -> START (всегда подгружаем новые файлы из /images)
    preloadImages().then(start);
},
handleError: function(err) {
    if (window.PGZ_STOP_REQUESTED || !PythonIDE.running || PythonIDE.runMode === 'finished') {
        console.log('Ignored runtime error after stop:', err);
        return;
    }
    PythonIDE.running = false;
        ...
        <div id="error-modal-overlay" class="error-modal-overlay">
            <div class="modal-error">
                <div class="error-modal-header">
                    <h3>${pgzUi('ide.errorModalTitle', null, 'Ошибка выполнения')}</h3>
                ...
                <div class="modal-footer">
                    <button class="btn-primary" id="error-modal-goto">${pgzUi('ide.gotoError', null, 'Перейти к ошибке')}</button>
                    <button class="btn-secondary" id="error-modal-close-footer">${pgzUi('common.close', null, 'Закрыть')}</button>
                </div>
            </div>
        </div>
    `;
    ...
    resetPgzRunGlobals: function() {
        if (typeof Sk === 'undefined' || !Sk.globals) return;
        var keys = [
        ...
        for (var i = 0; i < keys.length; i++) {
            delete Sk.globals[keys[i]];
        }
        if (window.GameModal && window.GameModal.resetChrome) {
            window.GameModal.resetChrome();
        }
    },
...
stop: function() {
    ...
    // Кеш картинок не сбрасываем здесь: последний кадр draw() ещё может выполниться.
    // Новый запуск всё равно очищает кеш в runCode().
    ...
    globals: ["abs","delattr","hash","memoryview","set","all","dict","help","min","setattr","any","dir","hex","next","slice","ascii","divmod","id","object","sorted","bin","enumerate","input","oct","staticmethod","bool","eval","int","open","str","breakpoint","exec","isinstance","ord","sum","bytearray","filter","issubclass","pow","super","bytes","float","iter","print","tuple","callable","format","len","property","type","chr","frozenset","list","range","vars","classmethod","getattr","locals","repr","zip","compile","globals","map","reversed","complex","hasattr","max","round"],
        ...
        return new Promise(function(accept) {
            ...
            if(addMore) {
                if (typeof EditorIDE !== 'undefined' && EditorIDE.PGZ_KEYWORDS) {
                    for (var p = 0; p < EditorIDE.PGZ_KEYWORDS.length; p++) {
                        addSuggestion(EditorIDE.PGZ_KEYWORDS[p], 'autocomplete-pgz');
                    }
                }
    ...
    builtinRead: function(file) {
        ...
        function loadFile(file) {
            ...
            if (file.name.match(/\.(txt|py|json)$/)) {
                ...
                if(file.name.match(/\.sb3$/)) {
                    PythonIDE.showHint(pgzUi('ide.sb3Attempt', null, 'Попытка прочитать файл sb3'));
                } else {
                    PythonIDE.showHint(pgzUi('ide.unsupportedFormat', null, 'Неподдерживаемый формат файла'));
                }
            }
        }
        ...
        if (typeof EditorIDE !== 'undefined') {
            EditorIDE.setup(PythonIDE.editor);
        }
                    ...
                    switch(e.currentTarget.id) {
                        case 'btn_file_rename':
                            ...
                            if(!newFileName.match(/^[A-Za-z0-9_.]+$/)){
                                PythonIDE.showHint(pgzUi('ide.invalidFileName', null, 'Некорректное имя файла'));
                                break;
                            }
                            if(PythonIDE.files[newFileName] || (PythonIDE.projectName + ".py" == newFileName)) {
                                PythonIDE.showHint(pgzUi('ide.fileExists', null, 'Файл с таким названием уже существует'));
                                break;
                            }
...
//
// Стили оболочки редактора и консоли (не путать с темой приложения)
applyCodeStyleChrome: function(style) {
    ...
    switch (style) {
        ...
        case 'dark':
            ...
            if (editorContainer) editorContainer.style.backgroundColor = '#0c1021';
            break;
        case 'dusk':
            document.body.style.backgroundColor = '#002240';
            if (editorContainer) editorContainer.style.backgroundColor = '#002240';
            break;
    }
},
applyOutputStyleChrome: function(style) {
    var output = document.getElementById('output');
    var consoleContainer = document.getElementById('consoleContainer');
    if (consoleContainer) {
        consoleContainer.style.backgroundColor = '';
        consoleContainer.style.color = '';
    }
    if (!output) return;
    output.classList.remove('output-style-light', 'output-style-dark', 'output-style-dusk');
    if (style === 'light') output.classList.add('output-style-light');
    else if (style === 'dark') output.classList.add('output-style-dark');
    else if (style === 'dusk') output.classList.add('output-style-dusk');
},
reapplyChromeStyles: function() {
    var codeStyle = PythonIDE.getOption('code_style', 'monokai');
    if (codeStyle === 'pro') codeStyle = 'monokai';
    PythonIDE.applyCodeStyleChrome(codeStyle);
    PythonIDE.applyOutputStyleChrome(PythonIDE.getOption('output_style', 'dark'));
},
//
// Инициализация настроек
initSettings: function() {
    ...
    if (sliderCodeSize && txtCodeSize) {
        const codeSize = PythonIDE.getOption('codeSize', 15);
        ...
        if (editorWrap) {
            editorWrap.style.setProperty('--code-font-size', codeSize + 'px');
        }
        if (editorEl) {
        ...
        sliderCodeSize.addEventListener('input', function() {
            if (typeof EditorIDE !== 'undefined' && EditorIDE.setFontSize) {
                EditorIDE.setFontSize(parseInt(this.value, 10));
            } else {
                txtCodeSize.value = this.value + "pt";
                if (editorWrap) editorWrap.style.setProperty('--code-font-size', this.value + 'px');
                if (editorEl) editorEl.style.fontSize = this.value + 'pt';
                PythonIDE.setOption('codeSize', this.value);
                if (PythonIDE.editor) PythonIDE.editor.refresh();
            }
        });
    }
    ...
    radioCodeStyleElements.forEach(function(radio) {
        radio.addEventListener('change', function() {
            if (this.checked) {
                const id = this.id;
                switch(id) {
                    case 'radio_code_style_monokai':
                        if (PythonIDE.editor) {
                            PythonIDE.editor.setOption("theme", "monokai");
                        }
                        PythonIDE.applyCodeStyleChrome('monokai');
                        PythonIDE.setOption('code_style', 'monokai');
                        break;
                    case 'radio_code_style_pro':
                        if (PythonIDE.editor) {
                            PythonIDE.editor.setOption("theme", "pgz-ide");
                        }
                        PythonIDE.applyCodeStyleChrome('pro');
                        PythonIDE.setOption('code_style', 'pro');
                        break;
                    case 'radio_code_style_light':
                        if (PythonIDE.editor) {
                            PythonIDE.editor.setOption("theme", "default");
                        }
                        PythonIDE.applyCodeStyleChrome('light');
                    ...
                    case 'radio_code_style_dark':
                        ...
                        PythonIDE.applyCodeStyleChrome('dark');
                    ...
                    case 'radio_code_style_dusk':
                        ...
                        PythonIDE.applyCodeStyleChrome('dusk');
    ...
    const radioCodeStyleChecked = document.getElementById('radio_code_style_' + codeStyle)
        || document.getElementById('radio_code_style_monokai');
...
//    
    resetFiles: function(all) {
        if(PythonIDE.aT && PythonIDE.hash && PythonIDE.aT[PythonIDE.hash] && PythonIDE.defaultSavedCode) {
            ...
            PythonIDE.editor.setValue(PythonIDE.files[PythonIDE.currentFile]);
            PythonIDE.updateFileTabs();
            PythonIDE.showHint(pgzUi('ide.restoreDefaultHint', null, 'Восстановить сохранённый код по умолчанию. Для отмены воспользуйтесь кнопкой восстановления кода.'));
        }
    }
};
