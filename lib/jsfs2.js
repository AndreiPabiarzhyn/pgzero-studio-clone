...
(function () {
  ...
  function _Class(name) {
    ...
    this._metadataCache = null; // кеш метаданных файловой системы
  }
  ...
  _Class.prototype.invalidateMetadataCache = function () {
    this._metadataCache = null;
  };

  // === Загрузка метаданных файловой системы ===
 ...
 _Class.prototype._splitPath = function (pathString) {
  ...
  // Удалить двойные разделители (esc+esc -> временный маркер)
...
_Class.prototype._joinPath = function (pathArray) {
  ...
  _Class.prototype.write = async function (filename, content, compress = this.compressionDefault) {
    ...
    // Сохранение содержимого в файловое хранилище
    const db = await this._dbPromise;
  ...
  _Class.prototype.rm = async function (path) {
    ...
    const deleteSubtree = (node) => {
      if (Array.isArray(node)) {
        // Файл: удалить из файлового хранилища
