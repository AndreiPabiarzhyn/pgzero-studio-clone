    ...
    async function initPlayPage() {
        ...
        try {
            ...
            if (id) {
                ...
                throw new Error('Не указана игра. Нужен параметр ?id=…');
            }
            ...
            console.error(err);
            var hint = id
                ? 'Игра не найдена. Если ты только что опубликовал её на этом компьютере — открой ссылку здесь же. Скоро ссылки будут работать для всех.'
