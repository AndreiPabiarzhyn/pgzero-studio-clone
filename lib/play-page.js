...
(function (global) {
    ...
    function startGame() {
        ...
        var code = (typeof PythonIDE !== 'undefined' && PythonIDE.files && PythonIDE.files['my_pgz.py'])
            ? PythonIDE.files['my_pgz.py']
            : '';
        if (typeof PGZProjectIO !== 'undefined' &&
            typeof PGZProjectIO.hasDrawFunction === 'function' &&
            !PGZProjectIO.hasDrawFunction(code)) {
            showPlayError(L(
                'play.noDraw',
                null,
                'В игре нет функции draw() — на экране ничего не рисуется. Открой Studio, добавь draw() и опубликуй снова.'
            ));
            return;
        }
        showState('game');
    ...
    async function initPlayPage() {
        ...
        try {
            ...
            var hint;
            if (portablePayload) {
                hint = L('play.linkTruncated', null, 'Не удалось открыть игру по ссылке. Возможно, ссылка обрезана при отправке — скопируй её заново целиком из Studio.');
            } else if (id && err.message === 'not_found') {
                hint = L('play.notFound', null, 'Игра не найдена. Подожди 1–2 минуты после публикации и открой ссылку снова. Если только что опубликовал — скопируй свежую ссылку из Studio.');
            } else if (err.message === 'no_draw') {
                hint = L('play.noDraw', null, 'В игре нет функции draw() — на экране ничего не рисуется. Открой Studio, добавь draw() и опубликуй снова.');
            } else if (err.message === 'empty_project' || err.message === 'invalid_pgz') {
                hint = L('play.emptyProject', null, 'Файл игры пустой или повреждён. Открой Studio, проверь код и ресурсы и опубликуй заново.');
            } else if (err.message === 'missing_game') {
                hint = L('play.missingGame', null, 'В ссылке нет данных игры. Открой Studio, нажми «Опубликовать» и скопируй ссылку.');
            } else if (id) {
                hint = L('play.notFound', null, 'Игра не найдена. Подожди 1–2 минуты после публикации и открой ссылку снова. Если только что опубликовал — скопируй свежую ссылку из Studio.');
            } else {
