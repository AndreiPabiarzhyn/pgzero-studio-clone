...
(function () {
  ...
  function gameGroupLabel(group) {
    return GAME_GROUP_LABELS[group] || group;
  }
  ...
  async function loadSoundLibrary() {
    ...
    if (!items.length || !gameSounds.length) {
      grid.innerHTML = '<div class="sound-library-status">Загрузка библиотеки…</div>';
      cats.innerHTML = '';
      try {
        var requests = [];
  ...
  function renderSoundLibrary() {
    ...
    list.forEach(function (sound) {
      ...
      playBtn.setAttribute('aria-label', 'Послушать ' + sound.name);
      ...
      if (sound.isGamePack && sound.importName) {
        var codeHint = document.createElement('div');
        codeHint.className = 'sound-library-code';
        codeHint.textContent = 'sounds.' + sound.importName + '.play()';
  ...
  async function previewSound(sound, element) {
    ...
    try {
      ...
      if (typeof message === 'function') {
        await message('', 'Не удалось воспроизвести звук');
      }
    }
  }
