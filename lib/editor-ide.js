...
(function () {
    ...
    function updateStatus(cm) {
        ...
        if (!pos) return;
        var c = cm.getCursor();
        pos.textContent = 'Стр. ' + (c.line + 1) + ', кол. ' + (c.ch + 1);
        if (sel) {
            var something = cm.somethingSelected();
            sel.textContent = something ? ('Выделено: ' + cm.getSelection().length + ' симв.') : '';
        }
        if (lang) {
            var mode = cm.getMode().name || 'text';
            lang.textContent = mode === 'python' ? 'Python · Pygame Zero' : mode;
        }
    }
    ...
    function bindToolbar(cm) {
        ...
        var runBtn = document.getElementById('ideBtnRun');
        ...
        if (runBtn) runBtn.addEventListener('click', function () {
            if (typeof PythonIDE !== 'undefined') PythonIDE.runCode();
        });
    ...
    function setup(cm) {
        ...
        cm.on('beforeChange', function (editor, change) {
            if (change.origin !== '+input' || change.text.length !== 1 || change.text[0].length !== 1) return;
            var ch = change.text[0];
            if (!pairs[ch]) return;
            var cur = editor.getCursor();
            var line = editor.getLine(cur.line);
            if (line.charAt(cur.ch) === pairs[ch]) {
                change.update(undefined, undefined, ['']);
                setTimeout(function () { editor.execCommand('goCharRight'); }, 0);
            } else {
                change.update(undefined, undefined, [ch + pairs[ch]]);
                setTimeout(function () { editor.execCommand('goCharLeft'); }, 0);
            }
        });
