/** Библиотека 8-bit спрайтов Kenney (CC0) для игр */
(function () {
  'use strict';

  var GAME_SPRITES_JSON = './assets/image-library/game-sprites.json';
  var loadedCatalogVersion = '';

  function assetVersion() {
    return (typeof PGZRUN_ASSET_VERSION !== 'undefined') ? PGZRUN_ASSET_VERSION : '1';
  }

  function gameSpritesJsonUrl() {
    return GAME_SPRITES_JSON + '?v=' + assetVersion();
  }

  var IMAGE_GROUP_LABELS = {
    all: 'Все',
    characters: 'Персонажи',
    enemies: 'Враги',
    items: 'Предметы',
    projectiles: 'Снаряды',
    effects: 'Эффекты',
    tiles: 'Тайлы',
    backgrounds: 'Фоны',
    ui: 'Интерфейс'
  };

  function L(key, params, fallback) {
    if (typeof PGZI18n !== 'undefined' && PGZI18n.uiText) {
      return PGZI18n.uiText(key, params, fallback);
    }
    return fallback != null ? fallback : key;
  }

  function groupLabel(key) {
    return L('library.imageGroups.' + key, null, IMAGE_GROUP_LABELS[key] || key);
  }

  var sprites = [];
  var spriteLabels = {};
  var loadedLabelsLang = '';
  var currentGroup = 'all';
  var searchQuery = '';
  var selectedSprite = null;

  function getEntryExt(entry) {
    if (entry.file) {
      var dot = entry.file.lastIndexOf('.');
      if (dot !== -1) return entry.file.slice(dot + 1).toLowerCase();
    }
    return 'png';
  }

  function resolveSpriteUrl(entry) {
    if (entry.source === 'local') {
      return './' + String(entry.file).replace(/^\.\//, '') + '?v=' + assetVersion();
    }
    throw new Error('Неизвестный источник: ' + entry.source);
  }

  function currentLang() {
    return (typeof PGZI18n !== 'undefined' && PGZI18n.getLang) ? PGZI18n.getLang() : 'ru';
  }

  async function loadSpriteLabels(lang) {
    var normalized = lang || currentLang();
    if (normalized === 'ru') {
      spriteLabels = {};
      loadedLabelsLang = 'ru';
      return;
    }
    if (loadedLabelsLang === normalized && Object.keys(spriteLabels).length) return;
    try {
      var url = './locales/sprite-labels.' + normalized + '.json?v=' + assetVersion();
      var response = await fetch(url, { cache: 'no-store' });
      if (!response.ok) throw new Error('HTTP ' + response.status);
      spriteLabels = await response.json();
      loadedLabelsLang = normalized;
    } catch (err) {
      console.warn('Sprite labels load failed', err);
      spriteLabels = {};
      loadedLabelsLang = normalized;
    }
  }

  function spriteDisplayName(entry) {
    if (currentLang() !== 'ru' && spriteLabels[entry.id]) {
      return spriteLabels[entry.id];
    }
    return entry.label;
  }

  function mapSpriteEntry(entry) {
    var ext = getEntryExt(entry);
    return {
      name: spriteDisplayName(entry),
      importName: entry.id,
      group: entry.group,
      url: resolveSpriteUrl(entry),
      ext: ext,
      key: entry.id
    };
  }

  async function openImageLibraryModal() {
    var modal = document.getElementById('imageLibraryModal');
    if (!modal) return;
    modal.style.display = 'flex';
    searchQuery = '';
    currentGroup = 'all';
    selectedSprite = null;
    var searchInput = document.getElementById('imageLibrarySearch');
    if (searchInput) searchInput.value = '';
    var importBtn = document.getElementById('imageLibraryImportBtn');
    if (importBtn) importBtn.style.display = 'none';
    await loadImageLibrary();
  }

  function closeImageLibraryModal() {
    selectedSprite = null;
    var modal = document.getElementById('imageLibraryModal');
    if (modal) modal.style.display = 'none';
    var importBtn = document.getElementById('imageLibraryImportBtn');
    if (importBtn) importBtn.style.display = 'none';
    document.querySelectorAll('#imageLibraryGrid .image-library-item.selected').forEach(function (el) {
      el.classList.remove('selected');
    });
  }

  async function loadImageLibrary() {
    var grid = document.getElementById('imageLibraryGrid');
    if (!grid) return;

    var catalogVersion = assetVersion();
    if (sprites.length && loadedCatalogVersion === catalogVersion) {
      await loadSpriteLabels(currentLang());
      renderImageLibrary();
      return;
    }

    grid.innerHTML = '<div class="sound-library-status">' + L('library.loadingSprites', null, 'Загрузка спрайтов…') + '</div>';
    try {
      var response = await fetch(gameSpritesJsonUrl(), { cache: 'no-store' });
      if (!response.ok) throw new Error('HTTP ' + response.status);
      sprites = await response.json();
      loadedCatalogVersion = catalogVersion;
      await loadSpriteLabels(currentLang());
    } catch (err) {
      grid.innerHTML = '<div class="sound-library-status sound-library-error">' + L('library.loadError', null, 'Не удалось загрузить библиотеку.') + '</div>';
      console.error(err);
      return;
    }
    renderImageLibrary();
  }

  var SEARCH_ALIASES = {
    'пуля': ['laser', 'missile', 'bolt', 'arrow', 'projectiles', 'spell_'],
    'пули': ['laser', 'missile', 'bolt', 'arrow', 'projectiles'],
    'снаряд': ['laser', 'missile', 'rocket', 'bolt', 'arrow', 'meteor', 'projectiles'],
    'ракета': ['rocket', 'missile', 'ракет'],
    'корабль': ['ship', 'космическ', 'enemy_'],
    'космос': ['space', 'ship', 'laser', 'meteor', 'astronaut', 'косм'],
    'рыцарь': ['knight', 'рыцар', 'dungeon_knight'],
    'скелет': ['skeleton', 'скелет', 'cultist'],
    'призрак': ['ghost', 'призрак', 'wisp'],
    'монстр': ['dungeon_', 'enemy_', 'ogre', 'zombie', 'vampire', 'slime', 'spider', 'demon', 'witch'],
    'файербол': ['fireball', 'fire_', 'firebomb', 'огнен'],
    'огонь': ['fire_', 'fireball', 'огнен'],
    'лазер': ['laser'],
    'магия': ['wizard', 'staff', 'bolt', 'orb', 'маг', 'laser'],
    'меч': ['sword', 'меч'],
    'лук': ['bow', 'arrow', 'лук', 'стрел']
  };

  function spriteMatchesQuery(sprite, q) {
    if (!q) return true;
    var haystack = (sprite.name + ' ' + sprite.importName + ' ' + sprite.group).toLowerCase();
    if (haystack.indexOf(q) !== -1) return true;
    var aliasKeys = Object.keys(SEARCH_ALIASES);
    for (var i = 0; i < aliasKeys.length; i++) {
      var alias = aliasKeys[i];
      if (q.indexOf(alias) === -1 && alias.indexOf(q) === -1) continue;
      var tokens = SEARCH_ALIASES[alias];
      for (var j = 0; j < tokens.length; j++) {
        if (haystack.indexOf(tokens[j]) !== -1) return true;
      }
    }
    return false;
  }

  function filteredSprites() {
    var q = searchQuery.trim().toLowerCase();
    return sprites.map(mapSpriteEntry).filter(function (sprite) {
      if (currentGroup !== 'all' && sprite.group !== currentGroup) return false;
      return spriteMatchesQuery(sprite, q);
    });
  }

  function renderImageGroups() {
    var container = document.getElementById('imageLibraryGroups');
    if (!container) return;
    container.innerHTML = '';
    Object.keys(IMAGE_GROUP_LABELS).forEach(function (group) {
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'sound-library-group' + (currentGroup === group ? ' active' : '');
      btn.textContent = groupLabel(group);
      btn.addEventListener('click', function () {
        currentGroup = group;
        renderImageLibrary();
      });
      container.appendChild(btn);
    });
  }

  function renderImageLibrary() {
    renderImageGroups();
    var grid = document.getElementById('imageLibraryGrid');
    if (!grid) return;

    var list = filteredSprites();
    grid.innerHTML = '';

    if (!list.length) {
      grid.innerHTML = '<div class="sound-library-status">' + L('library.notFound', null, 'Ничего не найдено') + '</div>';
      return;
    }

    list.forEach(function (sprite) {
      var item = document.createElement('div');
      item.className = 'sound-library-item image-library-item';
      item.dataset.key = sprite.key;
      item.dataset.group = sprite.group;
      if (selectedSprite && selectedSprite.key === sprite.key) {
        item.classList.add('selected');
      }

      var thumb = document.createElement('img');
      thumb.className = 'image-library-thumb';
      thumb.src = sprite.url;
      thumb.alt = sprite.name;
      thumb.loading = 'lazy';
      thumb.draggable = false;

      var label = document.createElement('div');
      label.className = 'sound-library-name';
      label.textContent = sprite.name;
      label.title = L('library.spriteCodeHint', { name: sprite.name, code: sprite.importName },
        sprite.name + ' → Actor(\'' + sprite.importName + '\')');

      item.appendChild(thumb);
      item.appendChild(label);
      item.addEventListener('click', function () {
        selectSprite(sprite, item);
      });
      grid.appendChild(item);
    });
  }

  function selectSprite(sprite, element) {
    selectedSprite = sprite;
    document.querySelectorAll('#imageLibraryGrid .image-library-item.selected').forEach(function (el) {
      el.classList.remove('selected');
    });
    if (element) element.classList.add('selected');
    var importBtn = document.getElementById('imageLibraryImportBtn');
    if (importBtn) importBtn.style.display = 'inline-block';
  }

  async function importSelectedImageFromLibrary() {
    if (!selectedSprite) return;

    try {
      if (typeof initFS === 'function') await initFS();
      var fsRef = window.jsfs || fs;
      if (!fsRef) throw new Error('Файловая система не готова');

      var response = await fetch(selectedSprite.url);
      if (!response.ok) throw new Error('Не удалось загрузить картинку');
      var blob = await response.blob();

      var dataUrlRaw = await new Promise(function (resolve, reject) {
        var reader = new FileReader();
        reader.onload = function () { resolve(reader.result); };
        reader.onerror = reject;
        reader.readAsDataURL(blob);
      });

      var ext = selectedSprite.ext || 'png';
      var filename = selectedSprite.importName + '.' + ext;
      var dataUrl = dataUrlRaw;
      if (typeof PGZStorageGuard !== 'undefined') {
        var optimized = await PGZStorageGuard.prepareImageDataUrl(dataUrlRaw, filename);
        dataUrl = optimized.dataUrl;
        if (typeof PGZStorageGuard.checkImageFile === 'function') {
          PGZStorageGuard.checkImageFile({ name: filename, size: blob.size }, true);
        }
        await PGZStorageGuard.warnIfStorageHigh();
      }

      var baseName = selectedSprite.importName;
      var name = baseName + '.' + ext;
      name = PGZImageAssets.normalizeImageFileName(name, dataUrl);
      await PGZImageAssets.writeImageReplacing(fsRef, name, dataUrl);

      if (typeof afterImageAssetsChanged === 'function') {
        await afterImageAssetsChanged();
      } else if (typeof refreshGallery === 'function') {
        await refreshGallery();
        if (typeof scheduleProjectSave === 'function') scheduleProjectSave();
      }
      if (typeof showGallery === 'function') await showGallery();

      var displayName = selectedSprite.name;
      closeImageLibraryModal();
      if (typeof message === 'function') {
        await message(
          L('library.imageAdded', null, 'Картинка добавлена'),
          L('library.imageAddedBody', { name: displayName, file: name, code: baseName },
            '«' + displayName + '» сохранена как ' + name + '.\n\nВ коде:\nhero = Actor(\'' + baseName + '\')')
        );
      }
    } catch (err) {
      console.error(err);
      if (typeof message === 'function') {
        await message('', L('library.importError', { detail: err.message || err }, 'Ошибка импорта:\n' + (err.message || err)));
      }
    }
  }

  function onSearchInput(e) {
    searchQuery = e.target.value || '';
    renderImageLibrary();
  }

  if (typeof window.addEventListener === 'function') {
    window.addEventListener('pgz:langchange', function () {
      loadSpriteLabels(currentLang()).then(function () {
        var modal = document.getElementById('imageLibraryModal');
        if (modal && modal.style.display === 'flex') renderImageLibrary();
      });
    });
  }

  window.openImageLibraryModal = openImageLibraryModal;
  window.closeImageLibraryModal = closeImageLibraryModal;
  window.importSelectedImageFromLibrary = importSelectedImageFromLibrary;

  function bindImageLibraryUi() {
    var search = document.getElementById('imageLibrarySearch');
    if (search && !search.dataset.bound) {
      search.dataset.bound = '1';
      search.addEventListener('input', onSearchInput);
    }
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bindImageLibraryUi);
  } else {
    bindImageLibraryUi();
  }

  window._imageLibraryTest = {
    mapSpriteEntry: mapSpriteEntry,
    resolveSpriteUrl: resolveSpriteUrl,
    getEntryExt: getEntryExt,
    spriteDisplayName: spriteDisplayName,
    setSpriteLabels: function (labels, lang) {
      spriteLabels = labels || {};
      loadedLabelsLang = lang || 'en';
    },
    IMAGE_GROUP_LABELS: IMAGE_GROUP_LABELS
  };
})();
