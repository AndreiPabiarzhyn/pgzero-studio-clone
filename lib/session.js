...
(function (global) {
    ...
    function refreshFooterStatus(state) {
        ...
        if (typeof PGZProjectGallery !== 'undefined' && PGZProjectGallery.getActiveSlot() !== null) {
            ...
            var slotLabel = typeof PGZProjectGallery.cellLabel === 'function'
                ...
                : t('slot.label', { n: slotIndex + 1 }, 'Ячейка ' + (slotIndex + 1));
        ...
        if (state === 'saving') {
            parts.push(t('session.saving', null, 'Сохранение…'));
            el.classList.add('is-saving');
        } else if (state === 'saved') {
            parts.push(t('session.saved', null, 'Сохранено ✓'));
            el.classList.add('is-saved');
        } else if (state === 'error') {
            parts.push(t('session.saveError', null, 'Ошибка сохранения'));
            el.classList.add('is-error');
        }

        el.textContent = parts.join(' · ');
        el.title = state === 'saved'
            ? t('session.savedTitle', null, 'Игра сохранена в браузере. Файл .pgz — для переноса на другой компьютер.')
            : '';
    }
    ...
    function save() {
        ...
        try {
            ...
            if (typeof message === 'function' && e && e.message) {
                message(t('toolbar.save'), e.message);
            }
            return false;
        }
    }
    ...
    function formatSavedAt(ts) {
        ...
        if (diff < 60000) return t('session.justNow', null, 'только что');
        if (diff < 3600000) {
            var mins = Math.floor(diff / 60000);
            return t('session.minsAgo', { n: mins }, mins + ' мин. назад');
        }
