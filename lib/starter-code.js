...
(function () {
  ...
  window.PGZ_buildBaseCode = function () {
    var hint = pyString(uiText(
      'starterCode.emptyHint',
      null,
      'Добавь свой код!'
    ));
    return (
      ...
      '    screen.draw.text("' + hint + '", (20, 20), fontsize=32, color="white")\n\n' +
      'pgzrun.go()\n'
    );
  };

  window.PGZ_BASE_CODE = window.PGZ_buildBaseCode();
  ...
  window.PGZ_getNewProjectCode = function () {
    ...
    return window.PGZ_buildBaseCode();
  };

  window._starterCodeTest = {
    uiText: uiText,
    pyString: pyString,
    buildBaseCode: window.PGZ_buildBaseCode,
