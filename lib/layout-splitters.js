...
(function () {
    ...
    function setSidebarVisible(visible) {
        ...
        var btn = document.getElementById('btn_toggle_sidebar');
        if (!right) return;
        ...
        if (btn) {
            btn.classList.toggle('tb-btn--active', visible);
            btn.title = visible ? 'Скрыть боковую панель' : 'Показать боковую панель';
            if (typeof PZIcon !== 'undefined') {
                PZIcon.setButtonIcon(btn, visible ? 'panel' : 'panelOff');
            }
        }
        refreshEditor();
    }
    ...
    function init() {
        ...
        if (btn) btn.addEventListener('click', function () {
            var right = document.getElementById('rightBar');
            setSidebarVisible(right && right.classList.contains('layout-right--hidden'));
        });
    }
