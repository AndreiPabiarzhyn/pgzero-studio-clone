...
function clearAllSelections() {
  ...
  // Снимаем класс 'selected' со всех элементов галереи
...
function selectGalleryFile(type, path) {
  // --- 1. Снимаем всё предыдущее выделение ---
  clearAllSelections();

  // --- 2. Устанавливаем новое выделение ---
  selected[type] = path;

  // --- 3. Показываем соответствующую панель управления ---
  let controlId;
  ...
  // --- 4. Добавляем класс 'selected' выбранному элементу ---
        ...
        try {
            ...
            for (const path of allFiles) {
                ...
                if (['images', 'sounds', 'music'].includes(folder)) {
                    ...
                    // Логика определения MIME-типа и запись через fs.write...
