...
async function importSelectedImage() {
  ...
  try {
    ...
    let name = PGZImageAssets.normalizeImageFileName(filename, dataUrl);
...
input.addEventListener('change', async (e) => {
  ...
  for (const file of files) {
    try {
      ...
      let name;
      if (type === 'image') {
        name = await PGZImageAssets.resolveUploadImageName(fs, file.name, dataUrl, usedImageBaseNames);
        await PGZImageAssets.writeImageReplacing(fs, name, dataUrl);
        imagesChanged = true;
      } else {
        name = file.name;
