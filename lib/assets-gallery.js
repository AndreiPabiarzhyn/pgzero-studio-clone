...
function loadCategory(category) {
  ...
  filesInCategory.forEach(file => {
    ...
    label.title = name + ' — выделите, чтобы скопировать';
...
async function refreshGallery() {
  if (fs && typeof fs.invalidateMetadataCache === 'function') {
    fs.invalidateMetadataCache();
  } else if (window.jsfs && typeof window.jsfs.invalidateMetadataCache === 'function') {
    window.jsfs.invalidateMetadataCache();
  }
  await renderImages();
  await renderSounds();
  await renderMusic(); 
...
function appendItemName(item, displayName) {
  ...
  nameEl.title = displayName + ' — выделите, чтобы скопировать';
...
async function exportGallery() {
  if (typeof PGZProjectIO === 'undefined') {
    await message('', 'Модуль экспорта не загружен.');
    return;
  }
