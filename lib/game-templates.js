...
(function (global) {
    ...
    function renderGrid() {
        ...
        grid.innerHTML = '';
        var tasks = TEMPLATES.map(function (template) {
            return loadTemplateAssets(template.id).then(function (assets) {
                ...
                if (assets.images && assets.images.length) {
                    thumbHtml = '<img class="pg-slot-thumb" src="' + assets.images[0].dataUrl + '" alt="">';
                }

                var meta = '';
                if (assets.images && assets.images.length) {
                    meta = assets.images.length + ' карт.';
                }
