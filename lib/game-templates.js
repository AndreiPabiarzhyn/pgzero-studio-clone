...
(function (global) {
    ...
    function templateDisplayName(template) {
        if (!template) return uiText('templates.example', null, 'Пример');
        var localized = uiText('templates.' + template.id, null, null);
        if (localized && localized !== 'templates.' + template.id) return localized;
        return template.labelRu || template.title;
    }
    ...
    async function applyTemplateContent(template) {
        ...
        if (nameInput) nameInput.value = templateDisplayName(template);
    ...
    async function confirmReplaceSlot(slotIndex, template) {
        ...
        return askConfirm(
            uiText('templates.replaceTitle'),
            uiText('templates.replaceBody', {
                slot: cellLabel(slotIndex).toLowerCase(),
                name: name,
                template: templateDisplayName(template)
            })
        );
    }
    ...
    async function applyTemplateToSlot(template, slotIndex) {
        ...
        if (typeof PythonIDE !== 'undefined' && PythonIDE.showHint) {
            PythonIDE.showHint(uiText('templates.applied', {
                template: templateDisplayName(template),
                slot: cellLabel(slotIndex).toLowerCase()
            }));
        }
    ...
    async function renderApplySlotPicker() {
        ...
        for (var i = 0; i < slots.length; i++) {
            ...
            var title = data ? escapeHtml(data.projectName || uiText('common.untitled', null, 'Без названия')) : uiText('slot.empty', null, 'Пусто');
            card.innerHTML =
                (active === i ? '<span class="pg-slot-ribbon">' + escapeHtml(uiText('slot.active', null, 'Сейчас тут')) + '</span>' : '') +
    ...
    async function refreshApplyModal(template) {
        ...
        if (hintEl) {
            hintEl.textContent = active !== null
                ? uiText('templates.hintReplace')
                : uiText('templates.hintPick');
        }
        ...
        if (btnCurrent) {
            if (active !== null) {
                ...
                var label = currentName && currentName.value.trim()
                    ...
                    : uiText('templates.currentGame', null, 'текущую игру');
                btnCurrent.style.display = '';
                btnCurrent.textContent = uiText('templates.writeHere', {
                    slot: cellLabel(active),
                    name: label
                });
            } else {
        ...
        if (btnNew) {
            if (emptySlots.length) {
                ...
                btnNew.textContent = uiText('templates.writeEmpty', {
                    slot: cellLabel(emptySlots[0]).toLowerCase()
                });
    ...
    function renderGrid() {
        ...
        TEMPLATES.forEach(function (template) {
            ...
            card.innerHTML =
                ...
                '</div>' +
                '<div class="pg-slot-title">' + escapeHtml(templateDisplayName(template)) + '</div>' +
    ...
    if (typeof global.addEventListener === 'function') {
        global.addEventListener('pgz:langchange', function () {
            if (pendingTemplate) {
                refreshApplyModal(pendingTemplate).catch(function (err) {
                    console.error('refreshApplyModal on lang change failed', err);
                });
            }
        });
    }
