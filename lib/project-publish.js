...
(function (global) {
    ...
    function showSuccessScreen(result) {
        ...
        if (noteEl) {
            if (result.short) {
                noteEl.textContent = 'Короткая ссылка — отправь друзьям, они смогут поиграть без установки.';
            } else {
                noteEl.textContent = 'Ссылка работает в любом браузере. Скопируй целиком и отправь друзьям.';
            }
        }
