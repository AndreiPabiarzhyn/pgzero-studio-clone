...
(function (global) {
    ...
    function showSuccessScreen(result) {
        ...
        if (noteEl) {
            if (result.short) {
                ...
                noteEl.textContent = 'Длинная ссылка — для коротких добавь GitHub token в lib/publish-config.js (см. комментарий в файле).';
            } else {
    ...
    async function ensurePublishToken() {
        ...
        var token = global.prompt(
            'Для коротких ссылок нужен GitHub token (сохранится в этом браузере).\n\n' +
            'github.com/settings/tokens → pgzero-studio-clone → Contents: Read and write.\n\n' +
            'Вставь token:'
        );
        if (!token || !String(token).trim()) {
            return false;
        }
        PGZPublishStatic.saveToken(String(token).trim());
        return PGZPublishStatic.canUpload();
    }

    function formatHostError(message) {
        message = message || '';
        if (message.indexOf('not_configured') >= 0) {
            return 'Не настроен GitHub token.';
        }
        if (message.indexOf('401') >= 0 || message.indexOf('403') >= 0) {
            return 'GitHub отклонил token. Создай новый PAT с правом Contents: Read and write.';
        }
    ...
    async function handlePublishClick() {
        ...
        try {
            if (typeof PGZPublishStatic !== 'undefined' && !PGZPublishStatic.canUpload()) {
                var hasToken = await ensurePublishToken();
                if (!hasToken && typeof message === 'function') {
                    await message('', 'Без token будет длинная ссылка (#pgz1=…).\n\nМожно опубликовать и так, или вставить token и повторить.');
                }
            }
            ...
            if (result.error === 'link_too_large') {
                ...
                var tokenHint = result.needToken
                    ? '\n\nДля коротких ссылок добавь GitHub token в lib/publish-config.js.'
                    : '';
