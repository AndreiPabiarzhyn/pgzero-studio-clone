...
(function (global) {
    ...
    function setModalUiMode(mode) {
        ...
        if (title) {
            title.textContent = mode === 'startup' ? 'Где хранить твою игру?' : 'Мои игры';
        }

        var hint = document.getElementById('projectGalleryHint');
        if (hint) {
            hint.textContent = mode === 'startup'
                ? 'У тебя 6 ячеек в браузере. Выбери пустую — начнёшь новую игру, или нажми на сохранённую — продолжишь.'
                : 'Переключись на другую игру. Текущая сохранится автоматически.';
        }
    ...
    async function removeFromSlot(index) {
        if (uiMode === 'startup') {
            ...
            var ok = await askConfirm(
                'Удалить игру',
                'Удалить «' + (data.projectName || 'Без названия') + '» из ' + cellLabel(index).toLowerCase() + '?'
            );
            if (!ok) return false;
        ...
        var okDel = await askConfirm(
            'Удалить игру',
            'Удалить «' + (dataSw.projectName || 'Без названия') + '» из ' + cellLabel(index).toLowerCase() + '?'
        );
