...
(function (global) {
    ...
    var galleryOp = Promise.resolve();

    function runGalleryOp(fn) {
        var op = galleryOp.then(function () { return fn(); });
        galleryOp = op.catch(function (err) {
            console.warn('PGZProjectGallery: operation failed', err);
        });
        return op;
    }
    ...
    async function applyProjectImpl(data, options) {
    ...
    function applyProject(data, options) {
        return runGalleryOp(function () {
            return applyProjectImpl(data, options);
        });
    }
    ...
    async function saveToSlotImpl(index, options) {
        options = options || {};
        syncEditorToFiles();
    ...
    function saveToSlot(index, options) {
        return runGalleryOp(function () {
            return saveToSlotImpl(index, options);
        });
    }

    function saveActiveSlotQuiet() {
        var idx = getActiveSlot();
        if (idx === null) return Promise.resolve(false);
        return runGalleryOp(function () {
            return saveToSlotImpl(idx, { skipOverwriteConfirm: true });
        });
    }
    ...
    async function handleSlotPick(index) {
        return runGalleryOp(async function () {
            ...
            if (uiMode === 'startup') {
                if (data) {
                    ...
                    await applyProjectImpl(data, { recoverAssetsFromFs: true });
                    setActiveSlot(index);
                    if (!hadAssets) {
                        await saveToSlotImpl(index, { skipOverwriteConfirm: true });
                    }
                } else {
            ...
            if (data) {
                ...
                if (!ok) return;
                if (active !== null) {
                    await saveToSlotImpl(active, { skipOverwriteConfirm: true });
                }
                await applyProjectImpl(data);
                setActiveSlot(index);
                if (typeof PGZSession !== 'undefined') PGZSession.setStatus('saved');
                return;
            }
    ...
    function openSwitchModal() {
        runGalleryOp(async function () {
            syncEditorToFiles();
            var idx = getActiveSlot();
            if (idx !== null) {
                await saveToSlotImpl(idx, { skipOverwriteConfirm: true });
            }
            await showSwitchModal();
        }).catch(function (err) {
            console.error('PGZProjectGallery: openSwitchModal failed', err);
        });
    }
