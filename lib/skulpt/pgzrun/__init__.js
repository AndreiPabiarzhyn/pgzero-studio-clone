var $builtinmodule = function(name) {
    document.getElementById("gameModal") && window.GameModal
        ? window.GameModal.open()
    ...
    var pgzMusicStopHook = null;
    var pgzTonesStopHook = null;

    function registerGameAudio(audio) {
        if (!audio) return;
        if (!window.__PGZ_gameAudio) window.__PGZ_gameAudio = new Set();
        window.__PGZ_gameAudio.add(audio);
    }

    function stopAllGameAudio() {
        if (window.__PGZ_gameAudio) {
            window.__PGZ_gameAudio.forEach(function (audio) {
                try {
                    audio.pause();
                    audio.currentTime = 0;
                    audio.loop = false;
            ...
            window.__PGZ_gameAudio.clear();
        }
        if (typeof pgzMusicStopHook === 'function') pgzMusicStopHook();
        if (typeof pgzTonesStopHook === 'function') pgzTonesStopHook();
    }
    window.PGZ_stopAllGameAudio = stopAllGameAudio;
    ...
    function updateCoordsFromProps(props, size, pos) {
        ...
        if (hanchor === null || hanchor === undefined) {
            hanchor = 0.5; // по умолчанию — центр по горизонтали
        }
        if (vanchor === null || vanchor === undefined) {
            vanchor = 0.5; // по умолчанию — центр по вертикали
        }
        ...
        // Позиция рассчитывается относительно точки привязки (anchor) 
        // независимо от наличия поворота
        props.x = x - hanchor * size.width;
        props.y = y - vanchor * size.height;
    }

    function getColor(c) {
        // особые оттенки цветов (как в Pygame Zero)
        var SPECIAL_COLORS = {
            'green': [0, 255, 0],
            'gray': [190, 190, 190],
            'purple': [160, 32, 240],
        ...
        // Обработка объектов Skulpt
        if (c && c.v !== undefined) {
            // Если это кортеж или список Skulpt — конвертируем в массив JS
            c = c.v.map(function(item) {
                return Sk.ffi.remapToJs(item);
            });
        }
        var type = typeof c;
        // строковые значения возвращаем как есть ===
        if (type === 'string') {
            // сначала проверяем особые цвета
            var lowerC = c.toLowerCase().replace(/\s+/g, '');
            if (SPECIAL_COLORS[lowerC]) {
                var col = SPECIAL_COLORS[lowerC];
                return "rgb(" + col[0] + ", " + col[1] + ", " + col[2] + ")";
            }
            // любую другую строку возвращаем без изменений
            // (наприклад: 'red', '#ff0000', 'rgb(255,0,0)', 'rgba(255,0,0,0.5)' тощо)
            return c;
        }
        // кортежи конвертируем в rgb 
        if (Array.isArray(c)) {
            var r = c[0];
            var g = c[1];
            var b = c[2];
...
//
    ...
    function levenshtein(a, b) {
        ...
        for (i = 1; i <= a.length; i++) {
            ...
            for (j = 1; j <= b.length; j++) {
                ...
                if (prev + (a.charAt(i - 1) === b.charAt(j - 1) ? 0 : 1) < val) {
                    val = prev + (a.charAt(i - 1) === b.charAt(j - 1) ? 0 : 1);
                }
                prev = row[j];
                row[j] = val;
            }
    ...
    function findSimilarImageName(name) {
        var keys = Object.keys(imageCache() || {});
        var best = null;
        var bestDist = 999;
        var i;
        for (i = 0; i < keys.length; i++) {
            var dist = levenshtein(name, keys[i]);
            if (dist < bestDist) {
                bestDist = dist;
                best = keys[i];
            }
        }
        return bestDist > 0 && bestDist <= 3 ? best : null;
    }

    function throwImageNotFound(name) {
        var jsName = typeof name === 'string' ? name : Sk.ffi.remapToJs(name);
        var hint = findSimilarImageName(jsName);
        var msg = "Image '" + jsName + "' not found.";
        if (hint) msg += " Did you mean '" + hint + "'?";
        throw new Sk.builtin.KeyError(msg);
    }

    function loadImage(name) {
        var jsName = Sk.ffi.remapToJs(name);
        var asset = imageCache()[jsName];
        if (!asset) {
            return null;
        }
        return asset;
    ...
    function requireImage(name) {
        ...
        var asset = imageCache()[jsName];
        if (!asset) {
            throwImageNotFound(jsName);
        }
        return asset;
    }
   
    // Получить текущие значения атрибута как список (1 или 2 элемента)
    function getCurrentValues(obj, attr) {
        let rawValue;
        if (obj.coords && !obj.attributes?.image) { // Rect
            // Для Rect читаем напрямую из coords с учётом привязки
            ...
            // Для Actor используем существующую функцию получения атрибута
            const pyAttr = Sk.ffi.remapToPy(attr);
            rawValue = Sk.ffi.remapToJs(getActorAttribute(obj, pyAttr));
        }
        // Формируем список результатов через push(), без предположений о ключе
        const result = [];
    ...
    // Применить значение к атрибуту (универсально)
    function applyValues(obj, attr, values) {
        if (obj.coords && !obj.attributes?.image) { // Rect
            // Для Rect обновляем напрямую через существующие функции
            updateRectAttribute(obj, attr, values[0]);
            // Если значений больше одного и атрибут поддерживает второй компонент (например, 'pos' -> 'y')
            // это будет обработано отдельными анимациями для каждого атрибута
        } else { // Actor
            // Для Actor формируем правильный тип данных
            let pyValue;
            if (values.length === 1) {
                pyValue = Sk.ffi.remapToPy(values[0]);
            } else {
                // Для двух значений создаём кортеж
    ...
    function unpackKWA(kwa) {
        var result = {};
        if (!kwa || !kwa.length) return result;
    ...
    function getKwarg(kwa, key) {
        if (!kwa || !kwa.length) return undefined;
        for (var i = 0; i < kwa.length; i += 2) {
            if (Sk.ffi.remapToJs(kwa[i]) === key) return kwa[i + 1];
        }
        return undefined;
    }

    function applyInitPosition(self, args, kwa, posArg) {
        var kwPositionOrder = [
            'topleft', 'topright', 'bottomleft', 'bottomright',
            'midtop', 'midleft', 'midbottom', 'midright',
            'center', 'pos', 'left', 'right', 'top', 'bottom', 'centerx', 'centery'
        ];
        var key;
        var value;
        var i;

        for (i = 0; i < kwPositionOrder.length; i++) {
            key = kwPositionOrder[i];
            value = args[key];
            if (value === undefined) value = getKwarg(kwa, key);
            if (value !== undefined) {
                updateActorAttribute(self, new Sk.builtin.str(key), value);
                return true;
            }
        }

        if (posArg !== undefined && posArg !== null && posArg !== Sk.builtin.none.none$) {
            updateActorAttribute(self, new Sk.builtin.str('pos'), posArg);
            return true;
        }
        return false;
    }
    ...
    var Animation = Sk.misceval.buildClass(s, function($gbl, $loc) {
        $loc.__init__ = new Sk.builtin.func(function(self, object, tween, duration, onFinished, targets) {
            ...
            // Сохраняем начальные значения для каждого атрибута
            self.startValues = {};
            self.endValues = {};
            self.deltas = {};
            ...
            // Отменяем предыдущие анимации для этих атрибутов
            for (var attr in self.targets) {
        ...
        $loc._stop = new Sk.builtin.func(function(self, complete) {
            ...
            self._running = false;
            // Если complete=True, устанавливаем конечные значения
            if (doComplete) {
                for (var attr in self.targets) {
                    applyValues(self.object, attr, self.endValues[attr]);
                }
            }
            // Удаляем анимацию из глобального объекта
            for (var attr in self.targets) {
                var animKey = self.object.id + "_" + attr;
        ...
        // Свойство on_finished (геттер и сеттер)
        ...
        // Внутренний метод для обновления анимации
    ...
    var animate = function(kwa, object) {
        ...
        // Извлекаем целевые атрибуты (все, кроме особых)
        var targets = {};
        ...
        // Создаём и возвращаем экземпляр Animation
    ...
    //
    function getNaturalSize(self) {
        var img = imageCache()[self.attributes.image];
        return img ? { w: img.width, h: img.height } : { w: 0, h: 0 };
    }

    function setActorSize(self, w, h) {
        self.attributes.width = w;
        self.attributes.height = h;
        self._customSize = true;
        updateAnchor(self);
    ...
    function updateRectFromXY(self) {
        ...
        if (!self._customSize) {
            var natural = getNaturalSize(self);
            a.width = natural.w;
            a.height = natural.h;
        }
        a.left = a.x;
...
// === Surface Class ===
    var Surface = Sk.misceval.buildClass(s, function($gbl, $loc) {
        $loc.blit = new Sk.builtin.func(function(self, source, dest, area, special_flags) {
            ...
            var i = imageCache()[source.actor.attributes.image];
...
// === ZRect Class ===
    Sk.globals.ZRect = Sk.globals.Rect = Sk.misceval.buildClass(s, function($gbl, $loc) {
        ...
        $loc.__init__ = new Sk.builtin.func(function(self) {
            ...
            self.anchorVal = {
                x: 0,
                y: 0
            }; // У прямоугольников привязка всегда в левом верхнем углу
            ...
            // Синхронизируем attributes с coords
            updateRectFromXY(self);
        });
            ...
            switch (jsName) {
                ...
                default:
                    if (self.attributes.hasOwnProperty(jsName)) {
                        ...
                        // Убеждаемся, что возвращаем объект Skulpt, а не число JS
        ...
        $loc.collidepoint = new Sk.builtin.func(function(self) {
            ...
            if (arguments.length === 3) {
                ...
                // collidepoint((x, y)) или другой объект с координатами
                var arg = arguments[1];
                if (arg.tp$name === 'tuple' || (arg.v && arg.v.length >= 2)) {
                    ...
                    // Объект с атрибутами x/y (например, другой Rect)
            ...
            // Проверка: точка внутри прямоугольника (включая левую и верхнюю границу,
            // но НЕ включая правую и нижнюю — как в оригинальном Pygame)
        ...
        $loc.contains = new Sk.builtin.func(function(self, other) {
            ...
            // Убеждаемся, что other — это Rect
            var otherRect = other;
            if (other.tp$name !== 'Rect') {
                // Пробуем конвертировать из кортежа
        ...
        // Метод inflate(dx, dy) или inflate((dx, dy)) — возвращает новый увеличенный прямоугольник
        $loc.inflate = new Sk.builtin.func(function(self) {
            ...
            // Вычисляем новые координаты (увеличиваем вокруг центра)
            var cx = (self.coords.x1 + self.coords.x2) / 2;
            var cy = (self.coords.y1 + self.coords.y2) / 2;
        ...
        // Метод inflate_ip(dx, dy) или inflate_ip((dx, dy)) — меняет прямоугольник «на месте»
        $loc.inflate_ip = new Sk.builtin.func(function(self) {
            ...
            updateRectFromXY(self); // обновляем атрибуты x, y, width, height
            return Sk.builtin.none.none$;
        });
    }, "Rect", []);
    ...
    function updateAnchor(self) {
        ...
        self.anchorVal.x = calculateAnchor(self.anchor[0], "x", w);
        self.anchorVal.y = calculateAnchor(self.anchor[1], "y", h);
    }
    ...
    var updateActorAttribute = function(self, name, value) {
        ...
        switch (name) {
            ...
            case 'anchor':
                ...
                updateAnchor(self);
                // Обновляем позицию с учётом новой привязки
                a.x = a.x - self.anchorVal.x;
                a.y = a.y - self.anchorVal.y;
                break;
            case 'flip_x':
            ...
            case 'angle':
                ...
                break;
            case 'size':
                setActorSize(self, pos[0], pos[1]);
                break;
            case 'width':
                a.width = Sk.ffi.remapToJs(value);
                self._customSize = true;
                updateAnchor(self);
                break;
            case 'height':
                a.height = Sk.ffi.remapToJs(value);
                self._customSize = true;
                updateAnchor(self);
                break;
            case 'images':
...
var getActorAttribute = function(self, name) {
    ...
    switch (lookupName) {
        ...
        case 'size': {
            return new Sk.builtin.tuple(Sk.ffi.remapToPy([a.width, a.height]));
        }
        case 'width':
            return Sk.ffi.remapToPy(a.width);
        case 'height':
            return Sk.ffi.remapToPy(a.height);
 ...
 Sk.globals.Actor = Sk.misceval.buildClass(s, function($gbl, $loc) {
    ...
    var init = function(kwa, self, name, posArg) {
        ...
        // Fallback when Skulpt calls __init__ without co_kwargs prepended array.
        ...
        var img = requireImage(self.attributes.image);
        self.attributes.width = img.width;
        self.attributes.height = img.height;

        if (args.size) {
            var sz = Sk.ffi.remapToJs(args.size);
            self.attributes.width = sz[0];
            self.attributes.height = sz[1];
            self._customSize = true;
        }

        updateAnchor(self);

        applyInitPosition(self, args, kwa, posArg);
    ...
    init.co_kwargs = true;
    init.co_varnames = ['self', 'name', 'pos'];
    init.co_argcount = 3;
    init.$defaults = [Sk.builtin.none.none$];
    $loc.__init__ = new Sk.builtin.func(init);
    if ($loc.__init__.func_code) {
        $loc.__init__.func_code.co_kwargs = true;
        $loc.__init__.func_code.co_varnames = init.co_varnames;
        $loc.__init__.func_code.co_argcount = init.co_argcount;
        $loc.__init__.func_code.$defaults = init.$defaults;
    }
    ...
    $loc.draw = new Sk.builtin.func(function(self) {
        ...
        var img = imageCache()[self.attributes.image];
        if (!img) throwImageNotFound(self.attributes.image);
    ...
    $loc.collidepoint = new Sk.builtin.func(function(self, pos) {
        ...
        if (!p || typeof p !== 'object' || p.length === undefined || p.length < 2) {
            throw new Sk.builtin.TypeError("collidepoint() argument must be (x, y) tuple");
        }
        return Sk.ffi.remapToPy(
    ...
    function isKeyPressed(key) {
        key = key.toLowerCase();
        // Enter и Return — одна клавиша (как в Pygame Zero)
        if (key === 'enter') key = 'return';
        return new Sk.builtin.bool(keysPressed[key] == true);
    }
...
var SurfacePainter = Sk.misceval.buildClass(s, function($gbl, $loc) {
     ...
     */
    /**
     * Convert any draw.text / draw.textbox value to a JS string via Python str().
     */
    function textToString(text) {
        return Sk.ffi.remapToJs(new Sk.builtin.str(text));
    }
    ...
    var textbox = function(kwa, self, text, box) {
        ...
        const jsText = textToString(text);
...
var Clock = Sk.misceval.buildClass(s, function($gbl, $loc) {
    ...
    clockMethods.schedule = $loc.schedule;
    clockMethods.schedule_unique = $loc.schedule_unique;
    clockMethods.schedule_interval = $loc.schedule_interval;
    clockMethods.unschedule = $loc.unschedule;
}, 'Clock', []);
var clockInstance = Sk.misceval.callsim(Clock);
Sk.globals.clock = clockInstance;

function exposeClockGlobal(name, argCount) {
    Sk.globals[name] = new Sk.builtin.func(function() {
        Sk.builtin.pyCheckArgs(name, arguments, argCount, argCount);
        var method = clockMethods[name];
        var args = [clockInstance];
        for (var i = 0; i < arguments.length; i++) {
            args.push(arguments[i]);
        }
        return Sk.misceval.callsim.apply(null, [method].concat(args));
    });
}

exposeClockGlobal('schedule', 2);
exposeClockGlobal('schedule_unique', 2);
exposeClockGlobal('schedule_interval', 2);
exposeClockGlobal('unschedule', 1);
    ...
    var Screen = Sk.misceval.buildClass(s, function($gbl, $loc) {
        ...
        $loc.blit = new Sk.builtin.func(function(self, image, position) {
            ...
            if (imageCache()[jsName]) {
                cx.drawImage(imageCache()[jsName], coords[0], coords[1]);
                return Sk.builtin.none.none$;
            }
    ...
    var MusicSystem = Sk.misceval.buildClass(s, function($gbl, $loc) {
        ...
        pgzMusicStopHook = stopMusicPlayback;
    ...
    // УНИВЕРСАЛЬНЫЙ ВЫЗОВ PYTHON HANDLER
    ...
    function setupGamepadListeners() {
        // Сопоставление кнопок геймпада с кодами клавиш
        const KEY_MAP = {
            'UP': 38, // ArrowUp
            'DOWN': 40, // ArrowDown
            'LEFT': 37, // ArrowLeft
        ...
        // Универсальный обработчик нажатия
        function handlePress(e) {
            ...
            // 1. Обновляем состояние клавиш
            keysPressed[keyName.toLowerCase()] = true;
            keysCodePressed.add(keyCode);
            // 2. Добавляем визуальную обратную связь
            btn.classList.add('pressed');
            // 3. Эмулируем событие keydown
        ...
        // Универсальный обработчик отпускания
        function handleRelease(e) {
            ...
            // 2. Снимаем визуальную обратную связь
            btn.classList.remove('pressed');
            // 3. Эмулируем событие keyup
    ...
    // Автоматический показ на мобильных устройствах при старте игры
    if (/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        setTimeout(gamepadShow, 500); // Небольшая задержка для загрузки интерфейса
    }
    //--------------------------------------------------------
    s.go = new Sk.builtin.func(function() {
        ...
        width = 800;
        height = 600;
        if (Object.prototype.hasOwnProperty.call(Sk.globals, 'WIDTH')) {
            width = Sk.ffi.remapToJs(Sk.globals.WIDTH);
        }
        if (Object.prototype.hasOwnProperty.call(Sk.globals, 'HEIGHT')) {
        ...
        if (titleEl) {
            ...
            if (Object.prototype.hasOwnProperty.call(Sk.globals, 'TITLE')) {
                title = Sk.ffi.remapToJs(Sk.globals.TITLE);
            } else {
        ...
        const modalContainer = document.getElementById('gameModalWindow');
        if (modalContainer && document.getElementById('gameCanvas')) {
            modalContainer.style.width = width + 'px';
            modalContainer.style.height = (height + 34) + 'px';
        }
        if (window.GameModal) window.GameModal.reposition();
        ...
        function runUserUpdate() {
            ...
            if (argCount > 0 || varNames.length > 0) {
                return Sk.misceval.callsimAsync(handlers, Sk.globals.update, Sk.ffi.remapToPy(dt));
            }
            return Sk.misceval.callsimAsync(handlers, Sk.globals.update);
        }

        function runUserDraw() {
            if (window.PGZ_STOP_REQUESTED) {
                return Promise.resolve();
            }
            if (!Sk.globals.draw) {
            ...
            // Очистка сразу перед draw(), а не параллельно с update — иначе экран мерцает чёрным
            if (cx) {
                cx.fillStyle = '#000000';
                cx.fillRect(0, 0, width, height);
            }
            return Sk.misceval.callsimAsync(handlers, Sk.globals.draw);
        }
        ...
        function buildMouseButtonArgs(paramNames, pos, button) {
            const args = [];
            for (let i = 0; i < paramNames.length; i++) {
                const name = Sk.ffi.remapToJs(paramNames[i]);
                if (name === 'pos') args.push(pos);
                else if (name === 'button') args.push(button);
            }
            return args;
        }

        function callMouseButtonHandler(func, pos, button) {
            let params = [];
            if (func.func_code && func.func_code.co_varnames) {
                params = func.func_code.co_varnames;
            }
            const args = buildMouseButtonArgs(params, pos, button);
            Sk.misceval.callsimAsync(handlers, func, ...args);
        }

        function handleMouseDown(e) {
            // Определяем кнопку мыши (для касания всегда левая)
            let mouseButton = 0;
            ...
            callMouseButtonHandler(Sk.globals.on_mouse_down, pos, button);
        }
        if (Sk.globals.on_mouse_up) {
            ...
            // Отмена касания (например, при потере фокуса)
        ...
        function handleMouseUp(e) {
            ...
            const button = mouseButton;
            callMouseButtonHandler(Sk.globals.on_mouse_up, pos, button);
        }
        if (Sk.globals.on_mouse_move) {
            // сохраняем предыдущие координаты как JS-числа
            let px = null;
            let py = null;
            // Миша
            ...
            function handleMouseMove(e) {
                ...
                //BUTTONS
                const buttonsList = [];
                // Для событий касания всегда считаем, что нажата левая кнопка
                ...
                for (let i = 0; i < params.length; i++) {
                    const name = Sk.ffi.remapToJs(params[i]);
                    switch (name) {
                        case 'pos':
                            args.push(pos);
                            break;
        ...
        PythonIDE.keyHandlers.push(function(e) {
            ...
            if (e.type === "keydown") {
                ...
                if (key === 'return') keysPressed.enter = true;
            ...
            if (e.type === "keyup") {
                ...
                if (key === 'return') keysPressed.enter = false;
