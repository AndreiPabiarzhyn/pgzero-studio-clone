    ...
    var init = function(kwa, self, name, posArg) {
        ...
        positionKeys.forEach(function(key) {
            if (args[key] !== undefined) {
                props[key] = Sk.ffi.remapToJs(args[key]);
            }
        });
        ['left', 'right', 'top', 'bottom', 'centerx', 'centery'].forEach(function(key) {
            if (args[key] !== undefined) {
                props[key] = Sk.ffi.remapToJs(args[key]);
            }
        });

        var posJs = posArg ? Sk.ffi.remapToJs(posArg) : null;
        var hasKwPosition = positionKeys.some(function(key) { return props[key] !== undefined; }) ||
        ...
        if (posJs && !hasKwPosition) {
            props.center = posJs;
        }

        if (posJs || hasKwPosition) {
            if (args.anchor) {
                props.anchor = [self.anchor[0], self.anchor[1]];
            }
            updateCoordsFromProps(
                props,
                [self.attributes.width, self.attributes.height],
                posJs && !hasKwPosition ? posJs : null
            );
            self.attributes.x = props.x;
            self.attributes.y = props.y;
        }
